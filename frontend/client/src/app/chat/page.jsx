...
const handleTyping = (e) => {
  ...
  return (
 <div className="w-[80%] flex flex-col bg-[#2A2B33ff]">
  {/* Header */}
  <div className="flex items-center justify-between shadow-[2px_0_15px_rgba(0,0,0,0.4)] p-3 border-b border-[#212529] bg-[#2C2A30] shadow-2xl">
    <div className="flex items-center gap-3">
      <img src="/user.png" alt="avatar" className="w-10 h-10 rounded-full" />
      <div className="text-white">
        <p className="font-semibold text-base flex items-center">{chatReceive}</p>
      </div>
    </div>
  ...
  {/* Messages */}
  <div className="flex-1 px-3 py-4 overflow-y-auto space-y-3 custom-scrollbar">
    {chatMsgs.map((msg, index) => (
      <div key={index} className={`flex ${msg.sender === authName ? 'justify-end' : 'justify-start'}`}>
        <div className="flex items-end gap-1">
          <div className={`max-w-[85%] px-4 py-2 text-sm shadow ${
            msg.sender === authName
  ...
  {/* Typing Box */}
  <div className="h-auto min-h-[70px] mt-2 px-2 mb-6 ml-5 mr-5 flex items-center bg-[#1f1f1f] shadow-[0_8px_24px_rgba(0,0,0,0.2)] border border-[#2a2a2a] rounded-xl">
    <form onSubmit={sendMsg} className="flex w-full items-center gap-3 py-2 px-2">
      ...
      <input
        ...
        placeholder="Message..."
        required
        className="flex-1 px-4 py-2 text-sm text-white bg-[#1f1f1f] placeholder-gray-400 rounded-lg border border-[#2a2a2a] focus:outline-none focus:ring-2 focus:ring-[#444] transition-all"
      />
